...
{
    if (!dir.exists(paste0('images/',name)))
        dir.create(paste0('images/',name),recursive=T,showWarnings=F)
...
###    sed1='s|src=""|src=""images/|'
    sed1=paste0('s|src=""|src=""images/',name,'/|')  
    ...
    dummy=file.copy(supfiles,paste0('images/',name),overwrite=T)
    meta
}
